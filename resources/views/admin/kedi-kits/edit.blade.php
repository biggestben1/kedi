        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Edit KEDI Kit #{{ $kediKit->id }}</h3>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.kedi-kits.update', $kediKit) }}" method="POST" id="kitForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select name="category" id="category" class="form-select" required>
                                    ...
                                    <option value="english" {{ old('category', $kediKit->category) === 'english' ? 'selected' : '' }}>English</option>
                                    <option value="french" {{ old('category', $kediKit->category) === 'french' ? 'selected' : '' }}>French</option>
                                </select>
                                @error('category')
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number" name="price" id="price" class="form-control" value="{{ old('price', $kediKit->price) }}" step="0.01" min="0" required>
                                @error('price')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number" name="quantity" id="quantity" class="form-control" value="{{ old('quantity', $kediKit->quantity ?? 0) }}" min="0" required>
                                @error('quantity')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" id="description" class="form-control" rows="3">{{ old('description', $kediKit->description) }}</textarea>
                            @error('description')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            <button type="submit" class="btn btn-primary">
                                <i class="fe fe-save"></i> Update Kit
                            </button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load existing KD numbers
    @foreach($kediKit->items as $item)
        addKdRow('{{ $item->kd_no }}', {{ $item->is_old ? 'true' : 'false' }}, '{{ $item->purchased_by_user_id ?? '' }}');
    @endforeach
