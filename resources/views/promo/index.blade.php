...
<head>
    ...
    <title>My Promo – {{ config('app.name') }}</title>
                ...
                <div class="container-fluid main-container">
                    <div class="d-flex">
                        ...
                        <div class="d-flex order-lg-2 ms-auto header-right-icons">
                            <button class="navbar-toggler navresponsive-toggler d-lg-none ms-auto" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent-promo" aria-controls="navbarSupportedContent-promo" aria-expanded="false" aria-label="Toggle navigation">
                                <span class="navbar-toggler-icon fe fe-more-vertical"></span>
                            </button>
                            <div class="navbar navbar-collapse responsive-navbar p-0">
                                <div class="collapse navbar-collapse" id="navbarSupportedContent-promo">
                                    <div class="d-flex order-lg-2">
                                        ...
                                        <div class="dropdown d-flex profile-1">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                                                ...
                                                @php
                                                    $promoRole = auth()->user()->role?->name;
                                                    $promoAdminLabel = match($promoRole) {
                                                        'reseller' => 'Reseller',
                                                ...
                                                @endphp
                                                <a class="dropdown-item" href="{{ in_array($promoRole, ['headquarters', 'branch', 'service_center', 'annex']) ? route('admin.pharmacy.dashboard') : route('admin') }}"><i class="dropdown-icon fe fe-settings"></i> {{ $promoAdminLabel }}</a>
                                                @endif
                ...
                <div class="app-sidebar">
                    ...
                    <div class="main-sidemenu">
                        ...
                        <ul class="side-menu">
                            ...
                            <li class="slide">
                                <a class="side-menu__item active" href="{{ route('promo.index') }}"><i class="side-menu__icon fe fe-gift"></i><span class="side-menu__label">My Promo</span></a>
                            </li>
                            <li class="slide">
                            ...
                            @if(auth()->user()->isSuperAdmin() || auth()->user()->role?->name === 'wholesale_staff' || auth()->user()->role?->name === 'reseller' || auth()->user()->role?->name === 'accountant' || auth()->user()->role?->name === 'dispatch' || auth()->user()->role?->name === 'headquarters' || auth()->user()->role?->name === 'branch' || auth()->user()->role?->name === 'service_center' || auth()->user()->role?->name === 'annex')
                            @php
                                $promoSideRole = auth()->user()->role?->name;
                                $promoSideAdminLabel = match($promoSideRole) {
                                    'reseller' => 'Reseller',
                                    'accountant' => 'Accountant Panel',
                                    'dispatch' => 'Dispatch Panel',
                                    'headquarters' => 'Admin Dashboard',
                            ...
                            @endphp
                            <li class="slide">
                                <a class="side-menu__item" href="{{ in_array($promoSideRole, ['headquarters', 'branch', 'service_center', 'annex']) ? route('admin.pharmacy.dashboard') : route('admin') }}"><i class="side-menu__icon fe fe-settings"></i><span class="side-menu__label">{{ $promoSideAdminLabel }}</span></a>
                            </li>
                            @endif
                ...
                <div class="side-app">
                    <div class="main-container container-fluid">
                        <div class="page-header">
                            <h1 class="page-title">My Promo</h1>
                            <div>
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="{{ url('/') }}">Home</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">My Promo</li>
                                </ol>
                            </div>
                        </div>
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                                <h3 class="card-title mb-0">Promo Items You Qualify For</h3>
                                <a href="{{ route('shop') }}" class="btn btn-sm btn-outline-primary"><i class="fe fe-shopping-bag me-1"></i> Back to Shop</a>
                            </div>
                            <div class="card-body p-0">
                                @if($promos->isEmpty())
                                    <p class="text-muted p-4 mb-0">You have no promo records yet. Promo lists are uploaded by Headquarters and matched to your account using your KD NO. If you have placed orders with your KD number, your promos will appear here once they have been uploaded.</p>
                                @else
                                        ...
                                        <table class="table table-hover mb-0">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>{{ $p->promo_name ?? '—' }}</td>
                                                    <td>{{ $p->shop_no ?? '—' }}</td>
                                                    <td><strong>{{ $p->promo_item ?? '—' }}</strong></td>
                                                    <td>{{ $p->quantity }}</td>
                                                    <td>{{ $p->promo_meta ?? '—' }}</td>
                                                </tr>
                                                @endforeach
                            ...
                            @if(!$promos->isEmpty() && $promos->hasPages())
