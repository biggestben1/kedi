...
<ul class="side-menu">
    ...
    @php
        ...
        $canPurchaseKit = auth()->user()->isSuperAdmin()
            || in_array(auth()->user()->role?->name, ['headquarters', 'branch', 'service_center', 'annex', 'reseller', 'customer', 'cashier', 'distributor'], true);
    @endphp
    @if($canRegisterKd)
    <li class="slide">
        <a class="side-menu__item {{ $active === 'kd-registration' ? 'active' : '' }}" href="{{ route('admin.kd.registration.create') }}"><i class="side-menu__icon fe fe-edit"></i><span class="side-menu__label">Register KD NO</span></a>
    </li>
    @endif
    @if($canPurchaseKit)
    <li class="slide">
        <a class="side-menu__item {{ $active === 'kedi-kits' ? 'active' : '' }}" href="{{ route('admin.kedi-kits.purchase.create') }}"><i class="side-menu__icon fe fe-box"></i><span class="side-menu__label">Purchase KEDI Kit</span></a>
    </li>
    @endif
    <li class="slide">
