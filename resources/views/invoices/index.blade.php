...
<head>
    ...
    <title>My Invoices – {{ config('app.name') }}</title>
                                    ...
                                    <div class="d-flex order-lg-2">
                                        ...
                                        <div class="dropdown d-flex profile-1">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                                                ...
                                                @php
                                                    $invRole = auth()->user()->role?->name;
                                                    $invAdminLabel = match($invRole) {
                                                        'reseller' => 'Reseller',
...
'service_center' => 'Service Center Admin',
                                            ...
                                            @endphp
                                            <a class="dropdown-item" href="{{ in_array($invRole, ['headquarters', 'branch', 'service_center', 'annex']) ? route('admin.pharmacy.dashboard') : route('admin') }}"><i class="dropdown-icon fe fe-settings"></i> {{ $invAdminLabel }}</a>
                                                @endif
                ...
                <div class="app-sidebar">
                    ...
                    <div class="main-sidemenu">
                        ...
                        <ul class="side-menu">
                            ...
                            @if(auth()->user()->isSuperAdmin() || auth()->user()->role?->name === 'wholesale_staff' || auth()->user()->role?->name === 'reseller' || auth()->user()->role?->name === 'accountant' || auth()->user()->role?->name === 'dispatch' || auth()->user()->role?->name === 'headquarters' || auth()->user()->role?->name === 'branch' || auth()->user()->role?->name === 'service_center' || auth()->user()->role?->name === 'annex')
                            @php
                                $invSideRole = auth()->user()->role?->name;
                                $invSideAdminLabel = match($invSideRole) {
                                    'reseller' => 'Reseller',
                                    'accountant' => 'Accountant Panel',
                                    'dispatch' => 'Dispatch Panel',
                                    'headquarters' => 'Admin Dashboard',
                            ...
                            @endphp
                            <li class="slide">
                                <a class="side-menu__item" href="{{ in_array($invSideRole, ['headquarters', 'branch', 'service_center', 'annex']) ? route('admin.pharmacy.dashboard') : route('admin') }}"><i class="side-menu__icon fe fe-settings"></i><span class="side-menu__label">{{ $invSideAdminLabel }}</span></a>
                            </li>
                            @endif
                    ...
                    <div class="main-container container-fluid">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body p-0">
                                @if($invoices->isEmpty())
                                    <p class="text-muted p-4 mb-0">You have no invoices yet.</p>
                                @else
                            ...
                            @if(!$invoices->isEmpty() && $invoices->hasPages())
